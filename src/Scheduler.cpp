...
TaskGraph Scheduler::splitTaskByHardwareNum(TaskGraph tg) {
  ...
  // update old tasks io and comp size
  for (auto ori_idx = 0; ori_idx < old_tasks.size(); ori_idx++) {
    ...
    old_tasks[ori_idx].comp_size += (sp_nr - 1) * old_tasks[ori_idx].punish_comp_size;
    old_tasks[ori_idx].in_size += (sp_nr - 1) * old_tasks[ori_idx].punish_io_size;
    old_tasks[ori_idx].out_size += (sp_nr - 1) * old_tasks[ori_idx].punish_io_size;
  }
  bool complete = false;
  while (!complete) {
    complete = true;
    for (auto pre_idx = 0; pre_idx < old_tasks.size(); pre_idx++) {
      for (auto suc_idx = 0; suc_idx < old_tasks.size(); suc_idx++) {
        if (tg.comm_size[pre_idx][suc_idx] >= 0) {
          ...
          if (pre_task.out_size != suc_task.in_size) {
            ...
            if (suc_task.op.type == CONCAT || suc_task.op.type == BINARY) {
              ...
              tg.comm_size[pre_idx][suc_idx] = max_size;
            }
          }
        }
      }
